...
if __name__ == "__main__":
    ...
    while not glfw.window_should_close(window):
        ...
        #yx aumenta al apretar la d y si la posición del auto es la máxima posición hasta ese momento
        if controller.is_w_pressed == True and maxposy == posy:
            y += 2*delta                   #y es un contador que aumenta a medida que el auto llega más lejos
            #print(y)

        n0 = n      #se crea n0 para saber si aumentó n 
        n = int(y)  #es el y truncado y se usa para crear basuras
        
        #si n aumentó se crea una nueva basura
        if n>n0:
            #print(n)
            zombieNode = sg.SceneGraphNode("zombie" + str(n))       #se le pone el indice al nombre del nodo ej: garbage0, garbage 1
