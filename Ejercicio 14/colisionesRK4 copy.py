...
class Circle:
    ...
    def action(self, gravityAceleration, deltaTime):
        # RK4 integration  
        self.velocity = self.velocityAction(self.position, deltaTime)
        k1 = self.velocity
        K1 = self.gravity(position)
        position2 = np.array([self.position[0]+deltaTime/2, self.position[1]+deltaTime*K1/2], dtype=np.float32)
        k2 = self.velocityAction(position2, deltaTime)
        K2 = self.gravity(position2)
        position3 = np.array([position[0]+deltaTime/2, position[1]+deltaTime*K2/2], dtype=np.float32)
        k3 = self.velocityAction(position3, deltaTime)
        K3 = self.gravity(position3)
        position4 = np.array([position[0]+deltaTime, position[1]+deltaTime*K3], dtype=np.float32)
        K4 = self.gravity(position4)
        k4 = self.velocityAction(position4, deltaTime)

        self.position += (deltaTime/6) * (k1+2*k2+2*k3+k4)
    
    def velocityAction(self, position, deltaTime):
        # RK4 integration
        K1 = self.gravity(position)
        position2 = np.array([position[0]+deltaTime/2, position[1]+deltaTime*K1/2], dtype=np.float32)
        K2 = self.gravity(position2)
        position3 = np.array([position[0]+deltaTime/2, position[1]+deltaTime*K2/2], dtype=np.float32)
        K3 = self.gravity(position3)
        position4 = np.array([position[0]+deltaTime, position[1]+deltaTime*K3], dtype=np.float32)
    ...
    def gravity(self, position):
        gravity = 1/(np.linalg.norm((position))**2)
        return gravity
