...
def createScene(pipeline):
    ...
    backGroundNode1 = sg.SceneGraphNode("background0")
    backGroundNode1.childs = [mountainsNode, windMillGroupNode]

    backGroundNode2 = sg.SceneGraphNode("background0")
    backGroundNode2.transform = tr.matmul([tr.translate(2.0, 0, 0)])
    backGroundNode2.childs = [mountainsNode, windMillGroupNode]

    backGroundNode0 = sg.SceneGraphNode("background0")
    backGroundNode0.transform = tr.matmul([tr.translate(-2.0, 0, 0)])
    backGroundNode0.childs = [mountainsNode, windMillGroupNode]

    # Nodo del background con todos los nodos anteriores
    backGroundNode = sg.SceneGraphNode("background")
    backGroundNode.childs = [backGroundNode0, backGroundNode1, backGroundNode2]

    # Nodo padre de la escena
    sceneNode = sg.SceneGraphNode("world")
    sceneNode.childs = [skyNode, sunNode, backGroundNode, highwayNode,lineNode]
